    ...
    <section class="py-20 bg-gray-50 overflow-hidden">
        <div class="container mx-auto px-4 lg:px-8">
            <div class="text-center mb-16">
                <h6 class="text-primary font-bold tracking-widest uppercase text-sm mb-2">{{ __('معرضنا المتجدد') }}</h6>
                <h2 class="text-4xl font-bold text-secondary">{{ __('ابحث عن') }} <span class="text-primary">{{ __('سيارتك') }}</span> {{ __('في متجرنا') }}</h2>
            </div>
    ...
    <section class="py-24 bg-white relative overflow-hidden">
        <div class="container mx-auto px-4 lg:px-8">
            <div class="flex flex-col lg:flex-row items-center gap-16">
                ...
                <div class="lg:w-1/2">
                    <h6 class="text-primary font-bold tracking-widest uppercase text-sm mb-4">{{ __('من نحن') }}</h6>
                    <h2 class="text-4xl font-bold text-secondary mb-8 leading-tight">
                        @if ($about?->title)
                            {!! str_replace('سيارتك', '<span class="text-primary">' . __('سيارتك') . '</span>', $about->title) !!}
                        @else
                            {{ __('نحن شريكك الموثوق في') }} <span class="text-primary">{{ __('رحلة شراء') }}</span> {{ __('سيارتك') }}
                        @endif
                    </h2>
                    <p class="text-gray-500 mb-8 leading-relaxed italic text-lg line-clamp-3">
                        {{ $about?->description_1 ?? __('نقدم تجربة استثنائية في عالم السيارات، حيث الجودة والمصداقية هي أساس عملنا. نوفر لك كافة التسهيلات لتمتلك سيارة أحلامك بكل يسر وسهولة.') }}
                    </p>
                    ...
                    <a href="{{ route('about') }}"
                        class="bg-secondary text-white px-8 py-3 rounded-xl font-bold hover:bg-gray-800 transition shadow-lg inline-block">{{ __('اقرأ المزيد') }}</a>
                </div>
            </div>
        </div>
    </section>
